...
<form class="container-2" method="post">
    ...
    <div class="text small">Select your Country</div>
      <div class="form">
          <select name="country" required>
            <option value="" disabled selected>Where are you from ?</option>
            {#each countries.sort((a, b) => a.name.localeCompare(b.name)) as { name, code }}
              <option value={code}><span class="emojie">{getFlagEmoji(code)}</span> {name}</option>
            {/each}
          </select>
      </div>
